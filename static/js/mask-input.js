 ...
 * mask-input.js
 * Andrew Keym
 * This unobtrusive plugin adds a mask to the field so it is
 * easier for the user to type in values. By default there are
 * three types of masks - numeric, alpha and alphanumeric.  You
 * can add an additional type as well. I'll show you an example
 * below.This plugin handles fixed width masks (SSN, phone, etc.) and
 * also numeric masks (which is basically dynamically adding the
 * comma between)
 *
 ...
 *
 * data-mask-input-def      This optional attribute holds the JSON object that defines
 *                          your new definition The definition should have a character key
 *                          and regular expression.
 *
 * data-mask-keep           This optional attribute allows you to preserve the mask on
 ...
 * <input type="text" name="ssn"
        ...
        data-mask-keep="true">
 * This mask example allows asterisks and numbers on the first six numbers
 * of an SSN number. Again you can add any character you like. In this case
 * we had to escape the asterisk for the regex to work correctly.
 * ========================================================= */
    ...
    * ===================== */
    ...
    $(function () {
        ...
        $('body').on("blur.mask", "[data-mask-input]:not([data-mask-input=number])", function () {
            if ($(this).mask('unmask') === "") {
                $(this).val("");
                $(this).trigger('mask-field-empty');
            }

            if ($(this).val() !== $(this).mask('get_value')) {
                $(this).change();  // TODO: this causes AJAX calls on page load for masked fields.  Fix.
                $(this).mask('set_value');
            }
        });
